            ...
            try:
                ...
                if item_id:
                    ...
                    if blocked_sites and successful:
                        ok_sites = ', '.join([s.get('site_id') for s in successful])
                        blocked = ', '.join(blocked_sites)
                        self.log(asin, f"Parcial: OK={ok_sites} | Bloq={blocked}", "WARNING")

                        # Guardar site_ids exitosos para excluirlos
...
class Pipeline:
    ...
    def print_final_report(self, results: Dict, elapsed_time: float):
        ...
        if results["failed"]:
            print(f"\n⚠️  Fallidos: {', '.join(results['failed'][:5])}")
            if len(results["failed"]) > 5:
                print(f"   +{len(results['failed']) - 5} más")
