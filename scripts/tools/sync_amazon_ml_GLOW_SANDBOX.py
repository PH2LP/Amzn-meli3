...
from src.integrations.amazon_glow_api_v2_advanced_SANDBOX import check_availability_v2_advanced
...
def get_all_published_listings():
    ...
    MODO SANDBOX: Si existe archivo 'asins_test.txt' en la raíz, solo procesa esos ASINs.
    """
    # MODO SANDBOX: Leer desde archivo si existe
    test_file = "asins_test.txt"
    if os.path.exists(test_file):
        print(f"   🧪 MODO TEST: Leyendo ASINs desde {test_file}", flush=True)
        with open(test_file, 'r') as f:
            test_asins = [line.strip() for line in f if line.strip() and not line.startswith('#')]
        print(f"   🧪 ASINs de prueba cargados: {len(test_asins)}", flush=True)
    else:
        test_asins = None
    ...
    if test_asins:
        # Filtrar por ASINs del archivo
        placeholders = ','.join('?' * len(test_asins))
        cursor.execute(f"""
            SELECT item_id, asin, price_usd, amazon_price_last, title, site_items
            FROM listings
            WHERE item_id IS NOT NULL AND asin IN ({placeholders})
            ORDER BY date_updated DESC
        """, test_asins)
    else:
        # Query normal - todos los listings
        cursor.execute("""
            SELECT item_id, asin, price_usd, amazon_price_last, title, site_items
            FROM listings
    ...
    if test_asins:
        print(f"   🧪 MODO TEST: {len(listings)} listings cargados (de {len(test_asins)} ASINs solicitados)", flush=True)
    else:
