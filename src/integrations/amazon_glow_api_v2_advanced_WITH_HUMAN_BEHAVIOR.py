...
# === CONFIGURACIÓN DE RATE LIMITING (COMPORTAMIENTO HUMANO) ===
# IMPORTANTE: Podés cambiar el perfil con la variable SCRAPING_PROFILE en .env
# Opciones: "SAFE", "BALANCED", "AGGRESSIVE"

PROFILE = os.getenv("SCRAPING_PROFILE", "BALANCED")

if PROFILE == "ULTRA_FAST":
    # PERFIL ULTRA RÁPIDO: Máxima velocidad (~3s por producto)
    # Ideal para: Muchos productos, cuenta establecida, riesgo aceptable
    # ADVERTENCIA: Usar curl_cffi es CRÍTICO aquí para evitar bloqueos
    MIN_DELAY = 1.0
    MAX_DELAY = 2.0
    MIN_REQUESTS_PER_SESSION = 80
    MAX_REQUESTS_PER_SESSION = 150
    SESSION_COOLDOWN_MIN = 15
    SESSION_COOLDOWN_MAX = 30
    PAGE_READ_TIME_MIN = 0.05
    PAGE_READ_TIME_MAX = 0.2
    GLOW_API_DELAY_MIN = 0.05
    GLOW_API_DELAY_MAX = 0.15
    GLOW_POST_DELAY = 0.8  # Delay después del Glow API POST
    GLOW_RETRY_DELAY = 0.5  # Delay entre reintentos de Glow
    ZIPCODE_ALT_DELAY = 0.3  # Delay para estrategia alternativa de zipcode
    VARIANT_DELAY = 0.5  # Delay al consultar variantes
    HUMAN_ERROR_PROBABILITY = 0.01  # Casi sin errores simulados
    print(f"🚀 Perfil ULTRA_FAST activado - Máxima velocidad (~3s/producto)")

elif PROFILE == "SAFE":
    # PERFIL SEGURO: Máxima protección, mínimo riesgo de bloqueo (~11s por producto)
    # Ideal para: Primera vez, cuentas nuevas, después de un bloqueo
    MIN_DELAY = 4.0
    MAX_DELAY = 10.0
    MIN_REQUESTS_PER_SESSION = 10
    MAX_REQUESTS_PER_SESSION = 25
    SESSION_COOLDOWN_MIN = 90
    SESSION_COOLDOWN_MAX = 240
    PAGE_READ_TIME_MIN = 1.2
    PAGE_READ_TIME_MAX = 3.0
    GLOW_API_DELAY_MIN = 0.8
    GLOW_API_DELAY_MAX = 2.0
    HUMAN_ERROR_PROBABILITY = 0.08  # Más errores = más humano
    print(f"🛡️  Perfil SEGURO activado - Máxima protección (~11s/producto)")

elif PROFILE == "AGGRESSIVE":
    # PERFIL AGRESIVO: Alta velocidad (~5s por producto)
    # Ideal para: Syncs urgentes, pocas iteraciones, cuenta establecida
    MIN_DELAY = 2.0
    MAX_DELAY = 4.0
    MIN_REQUESTS_PER_SESSION = 40
    MAX_REQUESTS_PER_SESSION = 80
    SESSION_COOLDOWN_MIN = 30
    SESSION_COOLDOWN_MAX = 60
    PAGE_READ_TIME_MIN = 0.3
    PAGE_READ_TIME_MAX = 0.8
    GLOW_API_DELAY_MIN = 0.2
    GLOW_API_DELAY_MAX = 0.5
    HUMAN_ERROR_PROBABILITY = 0.03
    print(f"⚡ Perfil AGRESIVO activado - Alta velocidad (~5s/producto)")

else:  # BALANCED (default)
    # PERFIL BALANCEADO: Balance entre velocidad y seguridad (~7s por producto)
    # Ideal para: Uso diario, syncs regulares, la mayoría de casos
    MIN_DELAY = 3.0
    MAX_DELAY = 6.0
    MIN_REQUESTS_PER_SESSION = 20
    MAX_REQUESTS_PER_SESSION = 50
    SESSION_COOLDOWN_MIN = 45
    SESSION_COOLDOWN_MAX = 120
    PAGE_READ_TIME_MIN = 0.5
    PAGE_READ_TIME_MAX = 1.5
    GLOW_API_DELAY_MIN = 0.3
    GLOW_API_DELAY_MAX = 1.0
    HUMAN_ERROR_PROBABILITY = 0.05
    print(f"⚖️  Perfil BALANCEADO activado - Balance velocidad/seguridad (~7s/producto)")
...
# === ADVANCED EVASION ===
# Rotar browser fingerprints (no solo chrome120)
...
class SessionRotator:
    ...
    def __init__(self):
        ...
        # ROTAR browser fingerprint (no siempre Chrome 120 - sospechoso)
        self.impersonate_browser = random.choice(BROWSER_FINGERPRINTS)
        # Generar un límite aleatorio para esta sesión (comportamiento humano)
    ...
    def get_session(self):
        ...
        # 1. No hay sesión
        # 2. Se excedió el límite aleatorio de requests
        if self.session is None or self.request_count >= self.session_request_limit:
            # Si hay sesión anterior, esperar cooldown VARIABLE (humanos no esperan tiempo fijo)
            if self.session is not None:
                cooldown = random.uniform(SESSION_COOLDOWN_MIN, SESSION_COOLDOWN_MAX)
                elapsed = time.time() - self.session_created_at
                if elapsed < cooldown:
                    wait = cooldown - elapsed
                    print(f"   🔄 Session rotation cooldown: {wait:.1f}s (comportamiento humano)...")
                    time.sleep(wait)
            ...
            # CAMBIO CRÍTICO: NO cargar cookies previas de browser
            # Las cookies se generarán automáticamente por curl_cffi al visitar Amazon
            # Esto es esencial para que el fingerprint sea consistente

            # OPCIONAL: Si hay zipcode configurado, hacer homepage visit para generar cookies
...
class RateLimiter:
    """
    Implementa rate limiting HUMANIZADO con:
    - Delays variables (humanos NO esperan tiempo fijo)
    - Distribución realista (70% rápido, 30% leyendo)
    ...
    def wait(self):
        """Espera el tiempo necesario antes del próximo request (comportamiento humano)"""
        ...
        # DISTRIBUCIÓN HUMANA DE DELAYS:
        # 70% del tiempo: usuario rápido (3-5 segundos)
        # 30% del tiempo: usuario leyendo (5-8 segundos)
        if random.random() < 0.7:
            # Usuario rápido - sabe lo que busca
            delay = random.uniform(MIN_DELAY, MIN_DELAY + 2.0)
        else:
            # Usuario leyendo - mirando detalles, reviews, etc
            delay = random.uniform(MIN_DELAY + 2.0, MAX_DELAY)
...
def check_availability_v2_advanced(asin: str, zipcode: str = None) -> Dict:
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            # EVASIÓN AVANZADA: Simular "error humano" ocasional
            # 5% de las veces, hacer un request "equivocado" primero (como humano que se confunde)
            if random.random() < HUMAN_ERROR_PROBABILITY:
                # Simular que el humano visitó homepage primero o buscó algo
                dummy_urls = [
                    "https://www.amazon.com",
                    f"https://www.amazon.com/s?k={asin[:5]}",  # Búsqueda incompleta
                ]
                dummy_url = random.choice(dummy_urls)
                try:
                    dummy_kwargs = {'timeout': 10}
                    if CURL_CFFI_AVAILABLE:
                        dummy_kwargs['impersonate'] = _session_rotator.impersonate_browser
                    _ = session.get(dummy_url, **dummy_kwargs)
                    time.sleep(random.uniform(0.3, 0.8))  # Breve pausa "humana"
                except:
                    pass  # Ignorar errores del dummy request
            ...
            if is_blocked_response(response.text, response.status_code):
                ...
                print(f"   🔍 DEBUG: HTML guardado: {debug_html}")
                print(f"   🔍 DEBUG: Metadata: {debug_json}")
                print(f"   🔍 DEBUG: Tamaño: {len(response.text)} bytes, CAPTCHA: {metadata['has_captcha']}, Price: {metadata['has_price']}")
            ...
            # SIMULAR COMPORTAMIENTO HUMANO: "Leer" la página antes de interactuar
            # Humanos NO cambian el zipcode inmediatamente - primero leen título, precio, etc
            page_read_time = random.uniform(PAGE_READ_TIME_MIN, PAGE_READ_TIME_MAX)
            ...
            # SIMULAR COMPORTAMIENTO HUMANO: Pequeño delay antes de cambiar zipcode
            # (humano hace scroll, mira opciones, etc)
            glow_delay = random.uniform(GLOW_API_DELAY_MIN, GLOW_API_DELAY_MAX)
            ...
            # EVASIÓN AVANZADA: Variar orden de extracción (humanos no siempre leen en mismo orden)
            # 50% del tiempo: primero precio, luego delivery
            # 50% del tiempo: primero delivery, luego precio
            if random.random() < 0.5:
                # Orden "normal": precio primero
                result["price"] = extract_price(html)
                time.sleep(random.uniform(0.1, 0.3))  # Breve pausa simulando lectura
                delivery = extract_delivery_info(html)
            else:
                # Orden "alternativo": delivery primero (como humano que busca envío rápido)
                delivery = extract_delivery_info(html)
                time.sleep(random.uniform(0.1, 0.3))  # Breve pausa simulando lectura
                result["price"] = extract_price(html)
            if delivery['found']:
