...
# Imports opcionales de ML (funciona con o sin ellos)
try:
    import numpy as np
    ...
    print("⚠️  sentence-transformers no disponible - usando modo fallback (solo IA)")
...
class CategoryDatabase:
    ...
    def __init__(self, cache_dir: str = "data/categories/cache"):
    ...
    def load_or_fetch_categories(self, force_refresh: bool = False) -> Dict:
        ...
        if not force_refresh and self._is_cache_valid():
            qprint("📦 Cargando categorías desde cache...")
            self.categories = self._load_from_cache()
            qprint(f"✅ {len(self.categories)} categorías CBT cargadas desde cache")
            return self.categories

        # Cache inválido o force refresh → descargar de ML
        qprint("🌐 Descargando categorías CBT desde MercadoLibre API...")
    ...
    def _fetch_from_ml_api(self) -> Dict:
        ...
        dump_file = Path("data/categories/cbt_categories_meta.json")
    ...
    def load_embeddings(self) -> Optional["np.ndarray"]:
        ...
        if self.embeddings_file.exists():
            try:
                ...
                qprint(f"📦 {len(self.embeddings)} embeddings cargados desde cache")
...
class EmbeddingMatcher:
    ...
    def __init__(self, database: CategoryDatabase):
        ...
        qprint(f"🤖 Cargando modelo de embeddings: {self.model_name}")
        self.model = SentenceTransformer(self.model_name)
        qprint("✅ Modelo de embeddings cargado")
    ...
    def _build_index(self):
        ...
        if cached_embeddings is not None and len(cached_embeddings) == len(self.database.categories):
            ...
            qprint("✅ Usando embeddings desde cache")
            return

        # No hay cache → calcular embeddings
        qprint("🔨 Calculando embeddings para todas las categorías...")
...
IMPORTANTE: Responde SOLO las palabras clave, nada más:"""
        ...
        try:
            ...
            qprint(f"   🔍 IA identificó tipo: '{keywords}'")
            return keywords
        ...
        if product.get('title'):
            ...
            if product.get('productType'):
                ...
                if product_type in type_mapping:
                    ...
                    qprint(f"   📦 ProductType hint: {product_type} → {type_mapping[product_type]}")
                else:
    ...
    def find_similar_categories(
        ...
        qprint(f"   🔍 Filtrando categorías para quedarse solo con LEAF (hojas)...")
    ...
    def _filter_leaf_categories(self, candidates: List[Dict]) -> List[Dict]:
        ...
        if parent_candidates:
            qprint(f"   🌿 Encontradas {len(parent_candidates)} categorías PADRE (no-leaf)")
            ...
            for parent in parent_candidates[:5]:  # Procesar top 5 padres
                ...
                qprint(f"      🔽 {cat_id} '{cat_name}' (sim: {parent_sim:.3f}) → Obteniendo hijos...")

                # Obtener hijos
                children_ids = self._get_category_children(cat_id)

                if children_ids:
                    qprint(f"         📁 {len(children_ids)} subcategorías encontradas")
                    ...
                    for child_id in children_ids:
                        if child_id in categories_db:
                            ...
                            if self._is_leaf_category(child_id):
                                ...
                                qprint(f"         ✅ Agregado hijo LEAF: {child_id} '{child_name}' (sim: {child_sim:.3f})")
                            else:
...
class CategoryMatcherV2:
    ...
    def __init__(self, cache_dir: str = "data/categories/cache"):
        ...
        # Solo inicializar embedder si tenemos las librerías de ML
        if HAS_ML:
            self.embedder = EmbeddingMatcher(self.database)
            qprint("✅ Embeddings habilitados (sentence-transformers disponible)")
        else:
            self.embedder = None
            qprint("⚠️  Embeddings deshabilitados - usando solo IA")
    ...
    def find_category(
        ...
        # ═══════════════════════════════════════════════════════════════════════════
        # REGLA DE FORZADO: LEGO siempre va a Building Toys
        # ═══════════════════════════════════════════════════════════════════════════
        brand = (product_data.get('brand') or '').upper()
        product_type = (product_data.get('product_type') or '').upper()
        title = (product_data.get('title') or '').upper()

        if 'LEGO' in brand or 'LEGO' in title or product_type == 'TOY_BUILDING_BLOCK':
            qprint("🧱 LEGO detectado → Forzando categoría Building Toys (CBT455425)")
            # Verificar que la categoría existe en la base de datos
            lego_category = self.database.get_category('CBT455425')
            if lego_category:
                processing_time = (time.time() - start_time) * 1000
                return {
                    'category_id': 'CBT455425',
                    'category_name': lego_category.get('name', 'Building Toys'),
                    'category_path': lego_category.get('path', 'Toys > Building Toys'),
                    'confidence': 1.0,  # 100% confianza en regla de forzado
                    'method': 'forced_rule_lego',
                    'reasoning': f'LEGO product (brand={brand}, type={product_type}) - forced to Building Toys',
                    'candidates_considered': 1,
                    'processing_time_ms': round(processing_time, 2)
                }
            else:
                print("⚠️ CBT455425 (Building Toys) no encontrada en DB, continuando con búsqueda normal...")

        # Fase 1: Similarity search con embeddings (si está disponible)
        if self.embedder:
            qprint(f"🔍 Fase 1: Buscando top {top_k} categorías similares...")
            ...
            # Sin embeddings, usar solo IA
            qprint("⚠️  Embeddings no disponibles - usando solo IA")
            candidates = []
            phase1_time = 0
        ...
        qprint(f"✅ Top {len(candidates)} candidatos encontrados (similarity: {candidates[0]['similarity_score']:.3f})")
        ...
        if use_ai:
            qprint("🤖 Fase 2: Validación con IA...")
        ...
        # Log resultado
        qprint(f"✅ Categoría seleccionada: {result['category_id']} ({result['category_name']})")
        qprint(f"   Confianza: {result['confidence']:.2f} | Método: {result['method']}")
        qprint(f"   Tiempo: {result['processing_time_ms']:.0f}ms")
