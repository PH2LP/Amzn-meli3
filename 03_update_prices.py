...
CONFIGURACIÓN EN .env:
    ...
    EXCLUDE_SITES=MLM            Marketplaces a excluir (opcional, ej: MLM o MLM,MLB)
...
OPCIONES:
    ...
    --exclude-sites MLM,MLB  Override: Excluir marketplaces (sobrescribe EXCLUDE_SITES del .env)
"""
...
def main():
    ...
    # Consultar precios de Amazon en batch (optimizado)
    amazon_prices = {}
    if not args.skip_amazon_check:
        log("🔍 Consultando precios actuales en Amazon SP-API...", Colors.CYAN)
        try:
            # Extraer todos los ASINs
            asins = [asin for asin, _, _, _, _ in listings]

            # La función get_prime_offers_batch_optimized obtiene el token internamente
            # y muestra progreso de batches automáticamente
            amazon_prices = get_prime_offers_batch_optimized(asins, batch_size=20, show_progress=True)
